...
function stock_widget_get_data($stock_list){
	...
	for($i=0;$i<3;$i++){
		...
		$response_list=stock_widget_url_get_json($url);
		...
		foreach($stock_list as $key){
			...
			if($data=='tickerDNE'||$data=='tickerInvalid'||is_null($data)||$data==''){
				...
				//valid stock list.
				$data_list=stock_widget_proccess_data($data);
...
function stock_widget_proccess_data($data_string){
	$key_list=array(
...
function stock_widget_url_get_json($Url) {
